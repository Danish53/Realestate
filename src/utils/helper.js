...
export const formatNumberWithCommas = (price) => {
  ...
  // Agar price string hai aur usme words hain (Crore, Lakh, Million etc.)
  if (typeof price === "string") {
    const hasText = /[a-zA-Z]/.test(price);

    if (hasText) {
      // Sirf currency symbol add karein, baqi text same rakhein
      return CurrencySymbol + " " + price;
    }

    // Agar string number hai (e.g. "25000000")
    price = Number(price.replace(/,/g, ""));
  }

  // Agar proper number hai
  if (!isNaN(price)) {
    const formattedPrice = formatNumberWithDynamicLocale(price, currencyCode);
    return CurrencySymbol + formattedPrice;
  }
